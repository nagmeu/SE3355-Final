...
app.post('/rate/:id', (req, res) => {
    ...
    db.query('INSERT INTO ratings (movie_id, user_id, rating) VALUES (?, ?, ?)', [movieId, req.session.user.id, rating], (err, result) => {
        if (err) {
            console.error('Rating ekleme hatası:', err);
            return res.status(500).json({ error: 'Rating ekleme hatası' });
        }
...
app.post('/watchlist/:movieId', (req, res) => {
    ...
    const userId = req.session.user.id; // Assuming the Google user's ID is stored in req.session.user.id
